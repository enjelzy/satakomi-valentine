...
export default function LoveMessage() {
  const compliments = [
    ...
    "Твоя улыбка — мое лучшее лекарство! 😊",
    "Ты делаешь меня счастливее с каждым днем! 💖",
    "Ты даришь мне крылья для полета! 🕊️",
    "Ты лучшее, что со мной случалось! 💗",
    "Ты — воплощение добра и красоты! 🌷",
    "Ты освещаешь мой мир ярче солнца! ☀️",
    "Ты прекрасна, как весенний рассвет! 🌸",
    "Ты наполняешь мое сердце любовью! 💕",
    "Ты — мой идеал! 💞",
    "Каждый миг с тобой — волшебство! ✨",
    "Ты как мелодия, наполняющая мое сердце! 🎶"
  ];
  ...
  if (startScreen) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-pink-300 to-red-400 text-white text-center">
        ...
        >
          💌 Лерочка, тебе пришла валентинка! 💌
        </motion.h1>
        <Button onClick={() => setStartScreen(false)} className="mt-6 bg-white text-red-500 px-6 py-2 rounded-full shadow-lg text-xl hover:bg-gray-200 transition-all">
          Открыть
        </Button>
      </div>
    );
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-gradient-to-r from-pink-300 to-red-400 relative overflow-hidden">
      ...
      >💞</motion.div>
      <Card className="p-6 shadow-xl bg-white rounded-2xl text-center w-80 relative z-10">
        <motion.div 
      ...
      <div className="mt-6">
        <Button onClick={handleClick} className="bg-red-500 hover:bg-red-600 text-white px-6 py-2 rounded-full shadow-lg transition-all transform hover:scale-110">
          Нажми меня
        </Button>
      </div>
      {showNote && (
        ...
        >
          ✨ Моя дорогая, ты самая особенная в моей жизни!✨
                      Special for Satakomi
          <motion.div 
